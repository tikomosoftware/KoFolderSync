...
$ProjectFile = "FolderSync.csproj"
$DistDir = "dist"
$TempFrameworkDir = "$DistDir\temp_framework"
$TempStandaloneDir = "$DistDir\temp_standalone"
$FrameworkZip = "$DistDir\KoFolderSync-v$Version-framework-dependent-release.zip"
$StandaloneZip = "$DistDir\KoFolderSync-v$Version-standalone-release.zip"
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        if (Test-Path "$TempFrameworkDir\FolderSync.exe") {
            Move-Item "$TempFrameworkDir\FolderSync.exe" "$TempFrameworkDir\KoFolderSync.exe" -Force
        }
        if (Test-Path "README.md") { Copy-Item "README.md" "$TempFrameworkDir\" -Force }
        @'
{
  "LastSourceFolder": "",
  "LastDestinationFolder": ""
}
'@ | Set-Content "$TempFrameworkDir\FolderSyncSettings.json.sample" -Encoding UTF8
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        if (Test-Path "$TempStandaloneDir\FolderSync.exe") {
            Move-Item "$TempStandaloneDir\FolderSync.exe" "$TempStandaloneDir\KoFolderSync.exe" -Force
        }
...
'@ | Set-Content "$TempStandaloneDir\FolderSyncSettings.json.sample" -Encoding UTF8
...
if ($fwSuccess -and (Test-Path $FrameworkZip)) {
    ...
    Write-Host "  Requires .NET 10.0 Runtime" -ForegroundColor Yellow
    Write-Host ""
}
